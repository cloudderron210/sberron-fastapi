...
async def patch_user(session: AsyncSession, 
                        user: User, 
                        user_update: PatchUser):
    
    update_data = user_update.model_dump(by_alias=True, exclude_unset=True)
    
    if 'telephone' or 'docRequisites' in update_data:
        await check_if_exists(session, user_update.telephone, user_update.docRequisites,  user_id=user.id)
